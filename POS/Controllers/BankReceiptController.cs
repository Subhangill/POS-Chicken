...
namespace POS.Controllers
{
	public class BankReceiptController : Controller
	{
		...
		public BankReceiptController(UserLog userLog, ApplicationDbContext applicationDbContext, IInterface @interface, IWebHostEnvironment webHostEnvironment)
		{
		...
		public IActionResult Index(string s_date, string e_date)
		{
			...
			var list = _repo.GetList<Voucher>("Select InvId,Id,Date,Detail,Cr From Voucher WHERE Type='BRV' and isdelete=0 and Date BETWEEN @start and @end ORDER BY Date,Id DESC", new { start = startdate.ToString("yyyy-MM-dd"), end = enddate.ToString("yyyy-MM-dd") }).ToList();
		...
		{
			voucher.InvId = _repo.GetSingleValue<int>("SELECT ISNULL(MAX(InvId),0)+1 FROM Voucher WHERE Type='BRV'"); ;

			voucher.Date = _dateTime;
			var vm = new VoucherVm()
			{
				Banklist = _repo.GetList<Account>("Select AccountNo,Name From Account WHERE SubHead=4 and HeadId in (1) and IsDelete=0 ORDER BY HeadId ,SubHead,AccountNo").ToList(),
				Voucher = voucher,
				Accountlist = _repo.GetList<Account>("Select AccountNo,Name From Account WHERE HeadId in (1,5) and IsDelete=0 ORDER BY HeadId ,SubHead,AccountNo").ToList()
			};
			return View(vm);
		}
		[HttpPost]
		public async Task<IActionResult> Save(VoucherVm voucherVm)
		{
			try
			{
				...
				if (isNew)
				{
					voucherVm.Voucher.InvId = _repo.GetSingleValue<int>("SELECT ISNULL(MAX(InvId),0)+1 FROM Voucher WHERE Type='BRV'"); ;
					voucherVm.Voucher.CreatedAt = _dateTime;
					voucherVm.Voucher.IsDelete = 0;
					voucherVm.Voucher.CreatedBy = _userId;
					voucherVm.Voucher.Type = "BRV";
				...
				else
				{
					...
					_context.TransactionDetail.Where(e => e.VType == "BRV" && e.InvNo == voucherVm.Voucher.InvId && e.TransId == voucherVm.Voucher.TranId).ExecuteDelete();
				...
				if (voucherVm.VoucherDetaillist != null && voucherVm.VoucherDetaillist.Count > 0)
				{
					foreach (var itm in voucherVm.VoucherDetaillist)
					{
						...
						_context.TransactionDetail.Add(new TransactionDetail { Accountno = itm.AccountId, TransId = tran_id, Date = voucherVm.Voucher.Date, Datetime = _dateTime, Dr = itm.Dr, Cr = itm.Cr, Detail = itm.Detail, InvNo = voucherVm.Voucher.InvId, VoucherNo = vno, InvType = "BRV", VType = "BRV" });
						if (!Convert.ToString(itm.AccountId).StartsWith("1"))
				...
				if (voucherVm.Voucher.ImageFile != null && voucherVm.Voucher.ImageFile.Length > 0)
				{
					...
					if (!isNew)
					{
						var existingImage = _context.ImagesDetail.FirstOrDefault(x => x.Recordid == voucherVm.Voucher.Id && x.Invtype == "Bank Receipt");
					...
					string fileName = $"BankReceipt_{voucherVm.Voucher.Id}{extension}";
					...
					_context.ImagesDetail.Add(new ImagesDetail
					{
						...
						Invtype = "Bank Receipt"
					});

					await _context.SaveChangesAsync();
				}
				TempData["save"] = isNew ? "Voucher saved successfully!!" : "Voucher updated successfully!!";
				_userlog.SaveHistory(form: "Bank Receipt", isNew ? "New" : "Edit", $"Id:{voucherVm.Voucher.Id}, Date:{voucherVm.Voucher.Date.ToString("dd-MM-yyyy")}, Note:{voucherVm.Voucher.Detail},Total Dr:{voucherVm.Voucher.Dr},Cr:{voucherVm.Voucher.Cr} ,Details==>{vdetails}");
				return RedirectToAction(actionName: "Index");
			}
			catch (Exception ex)
			{
				_repo.LogErrorToFile(ex, $"Error in Voucher ID: {voucherVm.Voucher.Id}");
				_userlog.SaveHistory("Bank Receipt", "Error", $"Error: {ex.Message.ToString()}");
				TempData["error"] = "An error occurred while making transaction.";
				voucherVm.Accountlist = _repo.GetList<Account>("Select AccountNo,Name From Account WHERE HeadId in (1,5) and IsDelete=0 ORDER BY HeadId ,SubHead,AccountNo").ToList();
		...
		public IActionResult Edit(int id)
		{
			...
			var imageDetail = _context.ImagesDetail.FirstOrDefault(x => x.Recordid == id && x.Invtype == "Bank Receipt");
			if (existing != null)
			{
			...
			{
				Banklist = _repo.GetList<Account>("Select AccountNo,Name From Account WHERE SubHead=4 and HeadId in (1) and IsDelete=0 ORDER BY HeadId ,SubHead,AccountNo").ToList(),
				VoucherDetaillist = _repo.GetList<VoucherDetail>("Select vd.*,Case WHEN a.SubHead=4 THEN (1) ELSE (0) END AS IsBank FROM VoucherDetail as vd LEFT JOIN Account as a on a.Accountno=vd.accountid WHERE vd.Voucherid=@id", new { id=id}).ToList(),
				Voucher = existing,
				Accountlist = _repo.GetList<Account>("Select AccountNo,Name From Account WHERE HeadId in (1,5) and IsDelete=0 ORDER BY HeadId ,SubHead,AccountNo").ToList()
			};
		...
		public IActionResult Delete(int id)
		{
			...
			_context.SaveChanges();
			_userlog.SaveHistory(form: "Bank Receipt", "Delete", $"Id:{existing.Id}, Date:{existing.Date.ToString("dd-MM-yyyy")}, Note:{existing.Detail},Total Dr:{existing.Dr},Cr:{existing.Cr} ,Details==>{vdetails}");
