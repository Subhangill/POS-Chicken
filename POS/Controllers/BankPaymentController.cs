...
namespace POS.Controllers
{
	public class BankPaymentController : Controller
	{
		...
		public BankPaymentController(UserLog userLog, ApplicationDbContext applicationDbContext, IInterface @interface, IWebHostEnvironment webHostEnvironment)
		{
		...
		public IActionResult Index(string s_date, string e_date)
		{
			...
			var list = _repo.GetList<Voucher>("Select InvId,Id,Date,Detail,Dr From Voucher WHERE Type='BPV' and isdelete=0 and Date BETWEEN @start and @end ORDER BY Date,Id DESC", new { start = startdate.ToString("yyyy-MM-dd"), end = enddate.ToString("yyyy-MM-dd") }).ToList();
		...
		{
			voucher.InvId = _repo.GetSingleValue<int>("SELECT ISNULL(MAX(InvId),0)+1 FROM Voucher WHERE Type='BPV'"); ;
		...
		public async Task<IActionResult> Save(VoucherVm voucherVm)
		{
			try
			{
				...
				if (isNew)
				{
					voucherVm.Voucher.InvId = _repo.GetSingleValue<int>("SELECT ISNULL(MAX(InvId),0)+1 FROM Voucher WHERE Type='BPV'"); ;
					voucherVm.Voucher.CreatedAt = _dateTime;
					voucherVm.Voucher.IsDelete = 0;
					voucherVm.Voucher.CreatedBy = _userId;
					voucherVm.Voucher.Type = "BPV";
				...
				else
				{
					...
					_context.TransactionDetail.Where(e => e.VType == "BPV" && e.InvNo == voucherVm.Voucher.InvId && e.TransId == voucherVm.Voucher.TranId).ExecuteDelete();
				...
				if (voucherVm.VoucherDetaillist != null && voucherVm.VoucherDetaillist.Count > 0)
				{
					foreach (var itm in voucherVm.VoucherDetaillist)
					{
						...
						_context.TransactionDetail.Add(new TransactionDetail { Accountno = itm.AccountId, TransId = tran_id, Date = voucherVm.Voucher.Date, Datetime = _dateTime, Dr = itm.Dr, Cr = itm.Cr, Detail = itm.Detail, InvNo = voucherVm.Voucher.InvId, VoucherNo = vno, InvType = "BPV", VType = "BPV" });
				...
				if (voucherVm.Voucher.ImageFile != null && voucherVm.Voucher.ImageFile.Length > 0)
				{
					...
					if (!isNew)
					{
						var existingImage = _context.ImagesDetail.FirstOrDefault(x => x.Recordid == voucherVm.Voucher.Id && x.Invtype == "Bank Payment");
					...
					string fileName = $"BankPayment_{voucherVm.Voucher.Id}{extension}";
					...
					_context.ImagesDetail.Add(new ImagesDetail
					{
						...
						Invtype = "Bank Payment"
					});

					await _context.SaveChangesAsync();
				}
				TempData["save"] = isNew ? "Voucher saved successfully!!" : "Voucher updated successfully!!";
				_userlog.SaveHistory(form: "Bank Payment", isNew ? "New" : "Edit", $"Id:{voucherVm.Voucher.Id}, Date:{voucherVm.Voucher.Date.ToString("dd-MM-yyyy")}, Note:{voucherVm.Voucher.Detail},Total Dr:{voucherVm.Voucher.Dr},Cr:{voucherVm.Voucher.Cr} ,Details==>{vdetails}");
				return RedirectToAction(actionName: "Index");
			}
			catch (Exception ex)
			{
				_repo.LogErrorToFile(ex, $"Error in Voucher ID: {voucherVm.Voucher.Id}");
				_userlog.SaveHistory("Bank Payment", "Error", $"Error: {ex.Message.ToString()}");
		...
		public IActionResult Edit(int id)
		{
			...
			var imageDetail = _context.ImagesDetail.FirstOrDefault(x => x.Recordid == id && x.Invtype == "Bank Payment");
			if (existing != null)
			{
		...
		public IActionResult Delete(int id)
		{
			...
			_context.SaveChanges();
			_userlog.SaveHistory(form: "Bank Payment", "Delete", $"Id:{existing.Id}, Date:{existing.Date.ToString("dd-MM-yyyy")}, Note:{existing.Detail},Total Dr:{existing.Dr},Cr:{existing.Cr} ,Details==>{vdetails}");
