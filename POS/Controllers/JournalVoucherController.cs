...
namespace POS.Controllers
{
	public class JournalVoucherController : Controller
	{
		...
		public JournalVoucherController(UserLog userLog, ApplicationDbContext applicationDbContext, IInterface @interface, IWebHostEnvironment webHostEnvironment)
		{
		...
		public IActionResult Index(string s_date, string e_date)
		{
			...
			var list = _repo.GetList<Voucher>("Select InvId,Id,Date,Detail,Dr,Cr From Voucher WHERE Type='JV' and isdelete=0 and Date BETWEEN @start and @end ORDER BY Date,Id DESC", new { start = startdate.ToString("yyyy-MM-dd"), end = enddate.ToString("yyyy-MM-dd") }).ToList();
		...
		public IActionResult Create(Voucher voucher)
		{
			...
			voucher.InvId = _repo.GetSingleValue<int>("SELECT ISNULL(MAX(InvId),0)+1 FROM Voucher WHERE Type='JV'"); ;

			var vm = new VoucherVm()
			{
				Voucher = voucher,
				Accountlist = _repo.GetList<Account>("Select AccountNo,Name From Account WHERE IsDelete=0 ORDER BY HeadId ,SubHead,AccountNo").ToList()
			};
			return View(vm);
		}
		[HttpPost]
		public async Task<IActionResult> Save(VoucherVm voucherVm)
		{
			try
			{
				...
				if (isNew)
				{
					voucherVm.Voucher.InvId = _repo.GetSingleValue<int>("SELECT ISNULL(MAX(InvId),0)+1 FROM Voucher WHERE Type='JV'"); ;
					voucherVm.Voucher.CreatedAt = _dateTime;
					voucherVm.Voucher.IsDelete = 0;
					voucherVm.Voucher.CreatedBy = _userId;
					voucherVm.Voucher.Type = "JV";
				...
				else
				{
					...
					_context.TransactionDetail.Where(e => e.VType == "JV" && e.InvNo == voucherVm.Voucher.InvId && e.TransId == voucherVm.Voucher.TranId).ExecuteDelete();
				...
				if (voucherVm.VoucherDetaillist != null && voucherVm.VoucherDetaillist.Count > 0)
				{
					foreach (var itm in voucherVm.VoucherDetaillist)
					{
						...
						_context.TransactionDetail.Add(new TransactionDetail { Accountno = itm.AccountId, TransId = tran_id, Date = voucherVm.Voucher.Date, Datetime = _dateTime, Dr = itm.Dr, Cr = itm.Cr, Detail = itm.Detail, InvNo = voucherVm.Voucher.InvId, VoucherNo = vno, InvType = "JV", VType = "JV" });
				...
				if (voucherVm.Voucher.ImageFile != null && voucherVm.Voucher.ImageFile.Length > 0)
				{
					...
					if (!isNew)
					{
						var existingImage = _context.ImagesDetail.FirstOrDefault(x => x.Recordid == voucherVm.Voucher.Id && x.Invtype == "Journal Voucher");
					...
					string fileName = $"JournalVoucher_{voucherVm.Voucher.Id}{extension}";
					...
					_context.ImagesDetail.Add(new ImagesDetail
					{
						...
						Invtype = "Journal Voucher"
					});

					await _context.SaveChangesAsync();
				}
				TempData["save"] = isNew ? "Voucher saved successfully!!" : "Voucher updated successfully!!";
				_userlog.SaveHistory(form: "Journal Voucher", isNew ? "New" : "Edit", $"Id:{voucherVm.Voucher.Id}, Date:{voucherVm.Voucher.Date.ToString("dd-MM-yyyy")}, Note:{voucherVm.Voucher.Detail},Total Dr:{voucherVm.Voucher.Dr},Cr:{voucherVm.Voucher.Cr} ,Details==>{vdetails}");
				return RedirectToAction(actionName: "Index");
			}
			catch (Exception ex)
			{
				_repo.LogErrorToFile(ex, $"Error in Voucher ID: {voucherVm.Voucher.Id}");
				_userlog.SaveHistory("Journal Voucher", "Error", $"Error: {ex.Message.ToString()}");
				TempData["error"] = "An error occurred while making transaction.";
				voucherVm.Accountlist = _repo.GetList<Account>("Select AccountNo,Name From Account WHERE IsDelete=0 ORDER BY HeadId ,SubHead,AccountNo").ToList();
		...
		public IActionResult Edit(int id)
		{
			...
			var imageDetail = _context.ImagesDetail.FirstOrDefault(x => x.Recordid == id && x.Invtype == "Journal Voucher");
			if (existing != null)
			{
		...
		public IActionResult Delete(int id)
		{
			...
			_context.SaveChanges();
			_userlog.SaveHistory(form: "Journal Voucher", "Delete", $"Id:{existing.Id}, Date:{existing.Date.ToString("dd-MM-yyyy")}, Note:{existing.Detail},Total Dr:{existing.Dr},Cr:{existing.Cr} ,Details==>{vdetails}");
